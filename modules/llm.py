...
class VideoScript:
    ...
    )  # 人物列表（CharacterInfo），用于 TTS 音色分配和外貌一致性
...
请直接输出 JSON，不要有任何其他文字。"""
    ...
    reflection_response = await client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "你是一位严格的视频脚本审核员。你只输出 JSON，不输出任何其他文字。",
            },
...
async def analyze_reference_video(
    ...
    try:
        ...
        if not gemini_api_key:
            raise ValueError(
                "Gemini API Key 未配置（需要在 image_gen.api_key 或 llm.gemini.api_key 中配置）"
            )
        ...
        for model_name in model_list:
            try:
                ...
                err_str = str(e)
                if (
                    "503" in err_str or "UNAVAILABLE" in err_str or "429" in err_str
                ) and "404" not in err_str:
                    if verbose:
